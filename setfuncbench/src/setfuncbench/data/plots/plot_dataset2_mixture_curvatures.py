...
Batch = Dict[str, Any]
DATASET_NAME = "dataset2_mixture_curvatures"
...
def plot_dataset2_mixture_curvatures_batch(
    batch: Batch,
    out_path: str,
    sample_index: int = 0,
    num_functions: int = 6,
    ...
    """Plot Dataset 2 curves for one sample and save a PNG."""
    try:
    ...
    g_ids = _get_latent_2d(batch, "g")
    outlier = _get_latent_2d(batch, "outlier")
    if outlier is not None:
        outlier = outlier.to(torch.bool)
    ...
    for k in range(n):
        ...
        if g_ids is not None:
            gid = int(g_ids[sample_index, k].item())
            color = cmap(gid % 10)
            group_text = f" g={gid}"
        if outlier is not None:
            is_out = bool(outlier[sample_index, k].item())
            marker = "x" if is_out else "o"
            out_text = f" out={int(is_out)}"
        ...
        ax.set_title(f"sample={sample_index} func={k}{group_text}{out_text}")
    ...
    fig.suptitle("dataset2_mixture_curvatures", fontsize=12)
    fig.tight_layout()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot dataset2_mixture_curvatures batch from a .pt file.")
    ...
    out = plot_dataset2_mixture_curvatures_batch(
        batch=batch,
        out_path=args.out,
