...
Batch = Dict[str, Any]
DATASET_NAME = "dataset3_hidden_pairing"
...
def plot_dataset3_hidden_pairing_batch(
    batch: Batch,
    out_path: str,
    sample_index: int = 0,
    num_functions: int = 6,
    ...
    """Plot Dataset 3 curves for one sample and save a PNG."""
    try:
    ...
    pair_id = _get_latent_2d(batch, "pair_id")
    ...
    for k in range(n):
        ...
        if pair_id is not None:
            pid = int(pair_id[sample_index, k].item())
            color = cmap(pid % 10)
            pair_text = f" pair={pid}"
        ...
        ax.set_title(f"sample={sample_index} func={k}{pair_text}")
    ...
    fig.suptitle("dataset3_hidden_pairing", fontsize=12)
    fig.tight_layout()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot dataset3_hidden_pairing batch from a .pt file.")
    ...
    out = plot_dataset3_hidden_pairing_batch(
        batch=batch,
        out_path=args.out,
